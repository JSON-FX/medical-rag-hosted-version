...
3. ~~Is one shared corpus enough to make the refusal path feel natural, or does the demo need a deliberately narrow corpus so out-of-scope questions are easy for a visitor to invent?~~ — **Answered: the narrow corpus, and it already exists.** Three FDA drug labels, ~71 chunks, with each drug's `verified_absent` axes measured over the assembled text rather than inferred from which sections were withheld. Withholding a section is not enough on its own — metformin's `dosage_and_administration` discusses paediatric dosing even with `pediatric_use` withheld — so the absence is scanned and pinned by a test. That gives the refusal path edges a visitor can find in under a minute, which is what G5 asks for.
4. ~~Should the telemetry panel be visible by default, or behind a toggle? Visible by default makes the engineering legible but adds noise for a non-technical reader.~~ — **Answered: visible by default.** The audience is a technical evaluator with ten minutes, and for that reader the strip is not noise, it *is* the argument — six tickets of gate, fusion and failover work are a paragraph in a README unless they can be watched happening. A toggle would also fail the refusal: the split telemetry exists so a decline arrives explained *before* its text renders, and an explanation behind a control the reader has not found yet is not an explanation. The cost is real but small — one compact strip per turn, below the answer and its sources, never above them.
